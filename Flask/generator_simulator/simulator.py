...
class Simulate:
    ...
    def initialize_infections(self, people_df):
        ...
        self.susceptible_dict[self.day_counter] = self.population - len(self.infected_set)
        return people_df
...
def simulate_epidemic(data):
    ...
    for day in sorted(sim.counter_date.keys()):
        if i == 100:
            break
        infected_counter["series"].append({
            "name": i,
            "value": int(sim.counter_date[day])
        })
        deaths += int(sim.death_dict[day])
