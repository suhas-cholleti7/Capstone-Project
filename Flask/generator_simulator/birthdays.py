...
class Birthdays:
    def __init__(self, data):
        ...
        self.school_prob = float(data["School Going Probability"])
        self.work_prob = float(data["Work Going Probability"])
        self.dog_walk_prob = float(data["Dog Walking Probability"])
        self.prayer_group_prob = float(data["Prayer Group Probability"])
        self.volunteer_prob = float(data["Volunteer Group Probability"])
        self.play_sports_prob = float(data["Play Sports Probability"])
        self.grocery_prob = float(data["Grocery Probability"])
        self.gas_prob = float(data["Gas Probability"])
        self.mall_prob = float(data["Mall Probability"])
    ...
    def initialize_infections(self, merged_df, children_df):
        ...
        self.susceptible_dict[str(_day)] = self.population - len(self.infected_set)
        return merged_df
    ...
    def initiate_birthdays(self, altered_merged_df, children_df, people_df):
        ...
        for day in self.day_list:
            ...
            self.susceptible_dict[str(day)] = self.population - len(self.infected_set)
...
def simulate_epidemic(data):
    ...
    for day in sorted(bir.counter_date.keys()):
        if i == 100:
            break
        infected_counter["series"].append({
            "name": i,
            "value": int(bir.counter_date[day])
        })
        # deaths += int(bir.death_dict[day])
        # death_counter["series"].append({
        #     "name": i,
        #     "value": deaths
        # })
        # i += 1
    res = []
    # res.append(death_counter)
    res.append(infected_counter)
