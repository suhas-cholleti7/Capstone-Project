...
class Birthdays:
    def __init__(self):
        ...
        self.date_done = set()
        self.vaccinated_families_set = set()
    ...
    def vaccinations(self, merged_df, family_df):
        """
        This function vaccinates 70% of the families who are responsible and who wear masks. These people will not be
        susceptible to the virus.
        :param merged_df: The merged dataframe
        :param family_df: The family dataframe
        :return:
        """
        percentage_of_vaccinated_families = 0.7
        irresponsible_df = family_df.loc[family_df["mask"] == 0]
        responsible_df = family_df.loc[family_df["mask"] == 1]
        self.vaccinated_families_set = set.union(set(irresponsible_df.sample(n=int(irresponsible_df.shape[0] * percentage_of_vaccinated_families))["fam_id"]), set(responsible_df.sample(n=int(irresponsible_df.shape[0] * percentage_of_vaccinated_families))["fam_id"]))
    ...
    def initialize_infections(self, merged_df, children_df):
        ...
        """
        _day = None
        children_df = children_df.loc[~(children_df["fam_id"].isin(self.vaccinated_families_set))]
        ...
        # birth_day_kids_df = birth_day_kids_df.loc[~birth_day_kids_df["fam_id"].isin(self.vaccinated_families_set)]
    ...
    def initiate_birthdays(self, altered_merged_df, children_df, people_df):
        ...
        for day in self.day_list:
            ...
            contagious_people_pot = contagious_people_pot.loc[~(altered_merged_df["fam_id"].isin(self.vaccinated_families_set))]
            ...
            random_selection_infected = [0.3, 0.4, 0.5, 0.6, 0.7]
            # children_df = children_df.loc[]
            contagious_kids_pot = children_df.loc[~(children_df["fam_id"].isin(self.vaccinated_families_set)) & (~children_df["person_id"].isin(self.infected_set)) & (children_df["person_id"].isin(kids_at_risk_set)) & (children_df["date"] == str(day))]
            sample_size = int(contagious_kids_pot.shape[0] * random.choice(random_selection_infected))
            ...
            if not contagious_kids_pot.empty:
                family_ids = set(contagious_kids_pot["fam_id"]).difference(self.vaccinated_families_set)
                ...
                self.infected_set = set.union(self.infected_set, inf_family)
                ...
                family_contact_df = family_contact_df.loc[~family_contact_df["fam_id"].isin(self.vaccinated_families_set)]
            ...
            while contagious_queue:
                ...
                cont_person_df = altered_merged_df.loc[~(altered_merged_df["fam_id"].isin(self.vaccinated_families_set)) & (altered_merged_df["person_id"].isin(set(cont_person["person_id"]))) & (altered_merged_df["date"] == str(day)) & (altered_merged_df["contagious"].isin([1]))]
                # Get all people who are not infected and are potential victims on the current day
                merged_without_infected = altered_merged_df.loc[(~altered_merged_df["fam_id"].isin(self.vaccinated_families_set)) & (~altered_merged_df["person_id"].isin(self.infected_set)) & (altered_merged_df["date"] == str(day)) & (~altered_merged_df["contagious"].isin([1]))]
                ...
                newly_exposed_df = newly_exposed_df.loc[~newly_exposed_df["fam_id"].isin(self.vaccinated_families_set)]
                altered_merged_df, counter = self.calculate_infection_metrics(day, counter, newly_exposed_df, altered_merged_df)
                newly_contagious = altered_merged_df.loc[(~altered_merged_df["fam_id"].isin(self.vaccinated_families_set)) & (~altered_merged_df["person_id"].isin(self.infected_set)) & (altered_merged_df["contagious"].isin([1])) & (altered_merged_df["date"].isin([day]))]
...
if __name__ == '__main__':
    if len(sys.argv) == 1:
        print("usage: python3 vaccinations_not_responsible_bday_v2.py [Argument_1]")
    ...
    bir.generate_dates()
    bir.vaccinations(merged_df, family_df)
    ...
    with open('../Results/Data/Vax/bday_infections_all_3.json', 'w') as convert_file:
        convert_file.write(json.dumps(bir.counter_date))
    # with open('../Results/Data/Vax_Not_Resp/new_infection_date_not_responsible_dict.json', 'w') as convert_file:
    #     convert_file.write(json.dumps(bir.new_infection_date_dict))
    # with open('../Results/Data/Vax_Not_Resp/recovery_date_not_responsible_dict.json', 'w') as convert_file:
    #     convert_file.write(json.dumps(bir.recovery_date_dict))
    # with open('../Results/Data/Vax_Not_Resp/susceptible_not_responsible_dict.json', 'w') as convert_file:
    #     convert_file.write(json.dumps(bir.susceptible_dict))
    # with open('../Results/Data/Vax_Not_Resp/death_not_responsible_dict.json', 'w') as convert_file:
    #     convert_file.write(json.dumps(bir.death_dict))
    # print(sum(sim.contact)/len(sim.contact))
    final_df.to_csv("../Results/Data/Vax/bday_irresponsible_simulation_df_all_3.csv", index=False)
