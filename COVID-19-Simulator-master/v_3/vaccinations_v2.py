...
class Simulate:
    def __init__(self):
        ...
        self.partially_vaccinated = set()
        self.completely_vaccinated = set()
        self.completely_vaccinated_dict = dict()
        self.partially_vaccinated_dict = dict()
        self.vaccintation_counter = collections.Counter()
    ...
    def initialize_infections(self, people_df):
        ...
        self.susceptible_dict[self.day_counter] = 36000 - len(self.infected_set)
        return people_df
    ...
    def start_simulations(self, merged_data_df):
        ...
        for day in self.day_list:
            ...
            # Get people to be vaccinated on current day -> They should not be in the infected set, partially vaccinated or completely vaccinated
            pre_vaccination_df = merged_data_df.loc[(~merged_data_df["person_id"].isin(self.infected_set)) & (~merged_data_df["person_id"].isin(self.completely_vaccinated_dict.keys()) & (~merged_data_df["person_id"].isin(self.partially_vaccinated_dict.keys()))) & (merged_data_df["date"] == str(day)) & (merged_data_df["age"] > 12)]
            if pre_vaccination_df.shape[0] > 400:
                pre_vaccination_df = pre_vaccination_df.sample(400)

            # Make them partially vaccinated
            for index, row in pre_vaccination_df.iterrows():
                self.partially_vaccinated_dict[row["person_id"]] = day
                full_vax_date = day + timedelta(days=self.vaccination_incubation)
                self.completely_vaccinated_dict[row["person_id"]] = full_vax_date
                self.vaccintation_counter[str(day)] += 1
            ...
            while contagious_queue:
                ...
                for index, infected in newly_exposed_df.iterrows():
                    ...
                    # Vaccination block starts here
                    day_in_datetime = datetime.strptime(infected["date"], '%Y-%m-%d').date()
                    if infected["person_id"] in self.completely_vaccinated_dict and infected["person_id"] in self.partially_vaccinated_dict:
                        if self.partially_vaccinated_dict[infected["person_id"]] <= day_in_datetime < self.completely_vaccinated_dict[infected["person_id"]]:
                            """
                            If the day of infection is between the day when vaccinated and the incubation period of vaccinations, the the person
                            can be infected
                            """
                            infection_flag = True
                            del self.completely_vaccinated_dict[infected["person_id"]]
                            del self.partially_vaccinated_dict[infected["person_id"]]
                            self.vaccintation_counter[str(day)] -= 1
                        elif (self.partially_vaccinated_dict[infected["person_id"]] < day_in_datetime) and (self.completely_vaccinated_dict[infected["person_id"]] < day_in_datetime):
                            """
                            If the day of infection is greater than both the vaccination date and the incubation dates, 
                            the person cannot be infected - condition for now
                            """
                            self.infected_set.add(infected["person_id"])
                    elif (infected["person_id"] not in self.completely_vaccinated_dict) or (infected["person_id"] not in self.partially_vaccinated_dict):
                        # If the person is not vaccinated at all, then the person can be infected with the virus
                        infection_flag = True
                    # Vaccination block ends here
...
if __name__ == '__main__':
    ...
    with open('../Results/Data/Vax/new_infection_date_vax_dict.json', 'w') as convert_file:
        convert_file.write(json.dumps(sim.new_infection_date_dict))
    with open('../Results/Data/Vax/recovery_date_vax_dict.json', 'w') as convert_file:
        convert_file.write(json.dumps(sim.recovery_date_dict))
    with open('../Results/Data/Vax/susceptible_vax_dict.json', 'w') as convert_file:
        convert_file.write(json.dumps(sim.susceptible_dict))
    with open('../Results/Data/Vax/death_vax_dict.json', 'w') as convert_file:
        convert_file.write(json.dumps(sim.death_dict))
    with open('../Results/Data/Vax/vaccinations_vax_dict.json', 'w') as convert_file:
        convert_file.write(json.dumps(sim.vaccintation_counter))
    # print(sum(sim.contact)/len(sim.contact))
    fully_merged_data_df.to_csv("../Results/Data/Vax/simulated_vax_df.csv", index=False)
