...
class FriendsInteraction:
    ...
    def create_friends(self, family_df):
        """
        ...
        :return: None
        """
        set_of_all_families = set(family_df["fam_id"])
        _friends = set()
        counter = 0
        ...
        for day in self.day_list:
            ...
            if day_counter == 6:
                ...
                for fam in families_that_meet:
                    people_of_families = set.union(people_of_families, self.friends_list[self.friends_set_index[fam]])
            ...
            altered_merged_df = altered_merged_df.loc[~altered_merged_df["date"].isin(self.date_done)]
            # altered_merged_df = altered_merged_df.loc[~altered_merged_df["person_id"].isin(self.infected_set)]
...
if __name__ == '__main__':
    if len(sys.argv) == 1:
        print("usage: python3 friends_interaction_v1.py [Argument_1]")
    ...
    with open('../Results/Data/Friends/infections_1.json', 'w') as convert_file:
        convert_file.write(json.dumps(frnd.counter_date))
    with open('../Results/Data/Friends/new_infection_date_friends_dict.json', 'w') as convert_file:
        convert_file.write(json.dumps(frnd.new_infection_date_dict))
    with open('../Results/Data/Friends/recovery_date_friends_dict.json', 'w') as convert_file:
        convert_file.write(json.dumps(frnd.recovery_date_dict))
    with open('../Results/Data/Friends/susceptible_friends_dict.json', 'w') as convert_file:
        convert_file.write(json.dumps(frnd.susceptible_dict))
    with open('../Results/Data/Friends/death_friends_dict.json', 'w') as convert_file:
        convert_file.write(json.dumps(frnd.death_dict))
    altered_merged_df.to_csv("../Results/Data/Friends/friends_df.csv", index=False)
