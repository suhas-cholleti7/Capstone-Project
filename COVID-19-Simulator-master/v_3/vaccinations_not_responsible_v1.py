...
class Simulate:
    ...
    def vaccinate_people(self, families_df, fully_merged_data):
        """
        To safegaurd a part of the population that is not responsible by vaccinating them.
        :param families_df: The Family Dataframe
        :param fully_merged_data: The Fully merged dataframe with all the tables combined
        :return: altered_fully_merged
        """
        rand_size_of_vaccinated = 0.5
        # size_of_vaccinated = [0.5]
        # rand_size_of_vaccinated = random.choice(size_of_vaccinated)
        print("Full data = ", fully_merged_data.shape)
        print("Size of vaccinated = ", rand_size_of_vaccinated)
        not_masked_families_df = families_df.loc[families_df["mask"] == 0]
        vaccinated_families_df = not_masked_families_df.sample(n=int(not_masked_families_df.shape[0] * rand_size_of_vaccinated))
        print("Families not masked = ", not_masked_families_df.shape)
        print("Vaccinated_Families = ", vaccinated_families_df.shape)
        vaccinated_set = set(vaccinated_families_df["fam_id"])
        susceptible_rows = fully_merged_data.loc[~fully_merged_data["fam_id"].isin(vaccinated_set)]
        print("Susceptible people = ", susceptible_rows.shape)
        return susceptible_rows
...
if __name__ == '__main__':
    if len(sys.argv) == 1:
        print("usage: python3 vaccinations_responsible_v1.py [Argument_1]")
    ...
    families_df = pd.read_csv("../Results/Data/family.csv")
    sim.generate_dates()
    altered_fully_merged = sim.vaccinate_people(families_df, fully_merged_data)
    fully_merged_data_df = sim.initialize_infections(altered_fully_merged)
    altered_fully_merged_data_df = sim.start_simulations(altered_fully_merged)
    print("The number of infected people are:", len(sim.infected_set))
    print("The simulator count is:", sim.simulator_count)
    with open('../Results/Data/Vax_Not_Resp/infections_not_responsible.json', 'w') as convert_file:
    ...
    altered_fully_merged_data_df.to_csv("../Results/Data/Vax_Not_Resp/not_responsible_simulation_df.csv", index=False)
