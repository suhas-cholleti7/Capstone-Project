...
class Simulate:
    ...
    def vaccinate_people(self, families_df, fully_merged_data):
        """
        To safegaurd a part of the population that is responsible by vaccinating them.
        ...
        masked_families_df = families_df.loc[families_df["mask"] == 1]
        vaccinated_families_df = masked_families_df.sample(n=int(masked_families_df.shape[0] * rand_size_of_vaccinated))
        print("Families not masked = ", masked_families_df.shape)
...
if __name__ == '__main__':
    ...
    with open('../Results/Data/Vax_Resp/infections_responsible.json', 'w') as convert_file:
        convert_file.write(json.dumps(sim.counter_date))
    with open('../Results/Data/Vax_Resp/new_infection_date_responsible_dict.json', 'w') as convert_file:
        convert_file.write(json.dumps(sim.new_infection_date_dict))
    with open('../Results/Data/Vax_Resp/recovery_date_responsible_dict.json', 'w') as convert_file:
        convert_file.write(json.dumps(sim.recovery_date_dict))
    with open('../Results/Data/Vax_Resp/susceptible_responsible_dict.json', 'w') as convert_file:
        convert_file.write(json.dumps(sim.susceptible_dict))
    with open('../Results/Data/Vax_Resp/death_responsible_dict.json', 'w') as convert_file:
        convert_file.write(json.dumps(sim.death_dict))
    # print(sum(sim.contact)/len(sim.contact))
    altered_fully_merged_data_df.to_csv("../Results/Data/Vax_Resp/responsible_simulation_df.csv", index=False)
